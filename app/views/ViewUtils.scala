...
object ViewUtils {
  ...
  def title(form: Form[_],
    ...
    titleNoForm(
      s"${errorPrefix(form)} ${messages(titleStr, titleMessageArgs: _*)}",
      section)
  ...
  def titleNoForm(title: String,
                  ...
                  titleMessageArgs: Seq[String] = Seq())(
      implicit messages: Messages): String =
    s"${messages(title, titleMessageArgs: _*)} - ${section.fold("")(
      messages(_) + " - ")}${messages("service.name")} - ${messages("site.govuk")}"
  ...
  def dateToStringWithoutYear(date: LocalDate)(
      implicit messages: Messages): String =
    s"${date.getDayOfMonth} ${messages(s"month.${date.getMonthValue}")}"
  def dateToStringOnlyMonth(date: LocalDate)(
      implicit messages: Messages): String =
