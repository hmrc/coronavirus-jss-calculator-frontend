...
private[mappings] class LocalDateFormatter(invalidKey: String)
  ...
  override def bind(
    ...
    def bindIntSubfield(subKey: String,
      ...
      intFormatter(
        ...
        .right
        .flatMap(int =>
          if (extraValidation(int)) Right(int)
          else Left(Seq(FormError(s"$key.$subKey", invalidErrorKey))))

    val dayField = bindIntSubfield("day",
                                   dayBlankErrorKey,
                                   dayInvalidErrorKey,
                                   day => day >= 1 && day <= 31)

    val monthField = bindIntSubfield("month",
                                     monthBlankErrorKey,
                                     monthInvalidErrorKey,
                                     month => month >= 1 && month <= 12)

    val yearField = bindIntSubfield("year",
                                    yearBlankErrorKey,
                                    yearInvalidErrorKey,
                                    year => year >= 1)
    ...
    (dayField, monthField, yearField) match {
      ...
      case (day, month, year) =>
        Left(
          day.left.toSeq.flatten ++ month.left.toSeq.flatten ++ year.left.toSeq.flatten)
    }
  }
