...
/** Service for encrypting data to send to Nuance (Virtual Assistance)
  ...
  */
class NuanceCryptoService @Inject() (configuration: Configuration) {

  private lazy val baseSettingsKey = "request-body-encryption"
  private lazy val hashingKey      = configuration.get[String](s"$baseSettingsKey.hashing-key")
  ...
  private lazy val hasher: Sha512Crypto = new Sha512Crypto(hashingKey)

  def encryptField(rawValue: String): String =
    "ENCRYPTED-" + crypto.encrypt(prefixWithHash(rawValue)).value
  ...
  private def prefixWithHash(rawValue: String): PlainText =
    PlainText(hashValue(rawValue).value + "-" + rawValue)
