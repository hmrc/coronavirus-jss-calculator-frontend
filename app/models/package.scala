...
package object models {
  ...
  implicit class RichJsValue(jsValue: JsValue) {
    ...
    private def setIndexNode(node: IdxPathNode,
                             oldValue: JsValue,
                             newValue: JsValue): JsResult[JsValue] = {
    ...
    private def removeIndexNode(
        node: IdxPathNode,
        valueToRemoveFrom: JsArray): JsResult[JsValue] = {
      val index: Int = node.idx
      ...
      valueToRemoveFrom match {
        case valueToRemoveFrom: JsArray
          ...
          val updatedJsArray = valueToRemoveFrom.value
            .slice(0, index) ++ valueToRemoveFrom.value
    ...
    private def setKeyNode(node: KeyPathNode,
                           oldValue: JsValue,
                           newValue: JsValue): JsResult[JsValue] = {
