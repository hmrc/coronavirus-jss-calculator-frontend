...
class TemporaryWorkingAgreementControllerSpec extends SpecBase with MockitoSugar {
  ...
  "TemporaryWorkingAgreement Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val request = fakeRequest(GET, temporaryWorkingAgreementRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[TemporaryWorkingAgreementView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(form.fill(TemporaryWorkingAgreement.values.head))(request, messages(application)).toString
      }
    }
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val request =
          fakeRequest(POST, temporaryWorkingAgreementRoute)
            .withFormUrlEncodedBody(("value", TemporaryWorkingAgreement.values.head.toString))
