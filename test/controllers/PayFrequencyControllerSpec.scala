...
class PayFrequencyControllerSpec extends SpecBase with MockitoSugar {

  private def onwardRoute = Call("GET", "/foo")

  private lazy val payFrequencyRoute = routes.PayFrequencyController.onPageLoad().url
  ...
  "PayFrequency Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val request = fakeRequest(GET, payFrequencyRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[PayFrequencyView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(form.fill(PayFrequency.values.head))(request, messages(application)).toString
      }
    }
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val request =
          fakeRequest(POST, payFrequencyRoute)
