...
import models.{ClaimPeriod, PayFrequency, PayPeriods, UserAnswers}
  ...
  "PayPeriodsList Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(form, List.empty, claimPeriod.yearMonth)(request, messages(application)).toString
      }
    }
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(form.fill(PayPeriods.values.head), List.empty, claimPeriod.yearMonth)(request, messages(application)).toString
      }
    }
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual
          view(boundForm, List.empty, claimPeriod.yearMonth)(request, messages(application)).toString
      }
    }
